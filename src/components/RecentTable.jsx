              ...
              {filteredOrders.map((order, idx) => (
                <tr key={order.order} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2 py-0.5 text-xs rounded-xl border ${
                      order.payment === 'Pending' 
                        ? 'bg-yellow-50 text-yellow-700 border-yellow-200' 
                        : 'bg-green-50 text-green-700 border-green-200'
                    }`}>
                      <span className={`mr-1 h-2 w-2 rounded-full ${
                        order.payment === 'Pending' ? 'bg-yellow-400' : 'bg-green-400'
                      }`}></span>
                      {order.payment}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2 py-0.5 text-xs rounded-xl border ${
                      ...
                      <span className={`mr-1 h-2 w-2 rounded-full ${
                        order.fulfillment === 'Unfulfilled' ? 'bg-red-400' : 'bg-green-400'
                      }`}></span>
                      {order.fulfillment}
                    </span>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap flex gap-4">
                    <span 
                      className="w-4 h-4 cursor-pointer rounded-full flex items-center justify-center text-gray-400" 
